...
def MusicTaggerCRNN(weights='msd', input_tensor=None):
    ...
    x = ELU()(x)
    x = MaxPooling2D(pool_size=(2, 2),strides=(2, 2), padding='same',name='pool1', trainable=False)(x)
    ...
    x = ELU()(x)
    x = MaxPooling2D(pool_size=(3, 3), strides=(3, 3),padding='same', name='pool2', trainable=False)(x)
    ...
    x = ELU()(x)
    x = MaxPooling2D(pool_size=(4, 4), strides=(4, 4),padding='same', name='pool3', trainable=False)(x)
    ...
    x = ELU()(x)
    x = MaxPooling2D(pool_size=(4, 4), strides=(4, 4),padding='same', name='pool4', trainable=False)(x)
    x = Dropout(0.1, name='dropout4', trainable=False)(x)

    # reshaping
    if K.image_data_format() == 'th':
        x = Permute((3, 1, 2))(x)
    x = Reshape((1, 128))(x)  #15,128
    ...
    if weights is None:
        ...
        initial_model.load_weights(r'C:\Users\priya\Downloads\Music-Genre-Classification-with-Deep-Learning-master\Music-Genre-Classification-with-Deep-Learning-master\models_trained\example_model\weights\crnn_net_gru_adam_ours_epoch_40.h5'.format(K.backend()),
                           by_name=True)
